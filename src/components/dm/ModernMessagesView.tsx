...
export const ModernMessagesView: React.FC<ModernMessagesViewProps> = React.memo(({
  user,
  profile,
  profiles = [],
  conversations,
  ...
  return (
    <div 
      ...
      style={useCustomTheme ? { color: customTheme.text_color } : {}}
    >
      {/* Conversations List */}
              ...
              {displayConversations.map(conv => {
                ...
                return (
                  <div
                    ...
                    className={`w-full p-4 rounded-2xl flex items-center gap-4 transition-all text-left relative group cursor-pointer select-none ${isActive ? 'bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 text-app-ink shadow-[0_0_20px_rgba(6,182,212,0.15)]' : 'bg-transparent border border-transparent hover:bg-white/5 hover:border-white/10 text-app-muted hover:text-app-ink'}`}
                  >
                    {isActive && (
                      <div 
                        className="absolute left-0 top-3 bottom-3 w-1.5 bg-gradient-to-b from-cyan-500 to-blue-500 rounded-r-full"
                      />
                    )}
